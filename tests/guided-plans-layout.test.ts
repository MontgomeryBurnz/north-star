...
test("Active Program review is split into state, cockpit, and team signal flows", () => {
  ...
  assert.match(teamSignalFlowSource, /data-active-program-team-execution/);
  assert.match(teamSignalFlowSource, /data-active-team-footprint-toggle/);
  assert.match(teamSignalFlowSource, /Role ownership is setup data/);
  assert.match(teamSignalFlowSource, /role="tablist"/);
  assert.match(teamSignalFlowSource, /role="tab"/);
  assert.match(teamSignalFlowSource, /aria-selected=\{selected\}/);
  assert.match(teamSignalFlowSource, /data-active-program-workspace-tab/);
  assert.match(teamSignalFlowSource, /data-active-team-execution-mode/);
  assert.match(teamSignalFlowSource, /Role update/);
  assert.match(teamSignalFlowSource, /Role update workspace/);
  assert.match(teamSignalFlowSource, /Progress board/);
  assert.match(teamSignalFlowSource, /Progress board workspace/);
  assert.match(teamSignalFlowSource, /Artifacts/);
  assert.match(teamSignalFlowSource, /Evidence workspace/);
