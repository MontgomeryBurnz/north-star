...
type AuthInvitePasswordFormProps = {
  email: string;
  initialName: string;
  inviteToken: string;
};

export function AuthInvitePasswordForm({ email, initialName, inviteToken }: AuthInvitePasswordFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/auth/user/activate-invite", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ displayName, inviteToken, password })
      });

      if (!response.ok) {
        const payload = (await response.json()) as { error?: string };
        throw new Error(payload.error ?? "Could not activate access.");
      }
  ...
  return (
    <main className="mx-auto flex min-h-[calc(100vh-12rem)] max-w-7xl items-center px-4 py-16 sm:px-6 lg:px-8">
      <div className="grid w-full gap-10 lg:grid-cols-[minmax(0,1.15fr)_minmax(24rem,30rem)] lg:items-center">
        <LoginBrandMark title="Activate North Star" subtitle="Where people, intelligence, and data move as one." />
          ...
          <CardContent className="grid gap-4 p-5">
            ...
            <form onSubmit={handleSubmit} className="grid gap-4">
              ...
              <Button type="submit" size="lg" disabled={isLoading}>
                ...
                {isLoading ? "Saving..." : "Activate access"}
              </Button>
            </form>
